...
class TransactionsExport implements FromCollection
{
    public function collection()
    {
        ...
        foreach ($transactions as $transaction) {
            ...
            $transactionArray[] = [$transaction->invois, $transaction->product->name, $transaction->reseller->name, $transaction->payment->bank_name, $transaction->buyers_name != null ? $transaction->buyers_name : '-', $transaction->quantity, $transaction->shipping, $transaction->address != null ? $transaction->address : '-', 'Rp. ' . number_format($transaction->price_per_product, 2, ',', '.'), 'Rp. ' . number_format($transactionPrice, 2, ',', '.'), 'Rp. ' . number_format($transaction->total_payment, 2, ',', '.'), $transaction->created_at];
        }

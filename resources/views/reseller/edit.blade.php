...
@section('content-dashboard')
    <div class="row me-lg-4 pb-4 d-none d-md-inline-block" style="margin-top: 32px;">
        <div class="col-12 pe-lg-0">
            ...
            @endif
            <form class="row row-cols-1" action="{{ route('reseller.update', $reseller->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method("PUT")
                <div class="col mb-3">
                ...
                <div class="col mb-3">
                    ...
                    <input required type="text" class="form-control @error('name') is-invalid @enderror" id="name" name="name" value="{{ old('name', $reseller->name) }}" value="{{ $reseller->name }}">
                    @error('name')
                        <div class="invalid-feedback">
                            {{ $message }}
                        </div>
                ...
                <div class="col mb-3">
                    ...
                    <input required type="email" class="form-control @error('email') is-invalid @enderror" id="email" name="email" value="{{ old('email', $reseller->user->email) }}" value="{{ $reseller->user->email }}">
                    @error('email')
                        <div class="invalid-feedback">
                            {{ $message }}
                        </div>
                ...
                <div class="col mb-3">
                    ...
                    <input required type="number" class="form-control @error('number_phone') is-invalid @enderror" id="number_phone" name="number_phone" value="{{ old('number_phone', $reseller->number_phone) }}" value="{{ $reseller->number_phone }}">
                    @error('number_phone')
                        <div class="invalid-feedback">
                            {{ $message }}
                        </div>
                ...
                <div class="col d-flex gap-2 mt-2">
                    ...
                    <a href="{{ route('reseller.index') }}" class="button-dark">Batal Edit</a>
                </div>
            </form>
        </div>
    </div>
